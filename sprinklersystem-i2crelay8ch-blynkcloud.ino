...
void setup() { //wished could delay loop() if zone on time is in the past on restart 1st zone turns on right away, but doesn't get recorded in table until its turned off
    ...
    // try below instead - Blynk.syncVirtual(V0 /*V1-autoCompleted V5, V6, V11 V12*/); // Dont sync master time V12.. sync Zone times V51-numZones
    setMode(off); //if battery fails, auto setup data will clear so set controllor to OFF
    modeBeforePowerFail = off;
...
void hourloop() {
    lasthour = Time.hour();
    if(hadVUSB) {pwLED.on();} else {pwLED.off();}
